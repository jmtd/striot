...
-- so we can return an expression (or something else? A list of decls?)
-- need to still manage Strings since imports cannot be generated by TH.
-- Perhaps this function should flatten the Q stuff down to a String.
--
-- lots more helper functions at the top-level, with a naming convention

importsToString :: GenerateOpts -> String
importsToString opts = concatMap (\i -> "import "++i++"\n") (imports opts)

nicerGenerateCodeFromStreamGraph :: StreamGraph -> Int -> GenerateOpts -> Q String
nicerGenerateCodeFromStreamGraph sg partId opts = return $ concat
    [ "-- node" ++ (show partId)
    , importsToString opts
    ]
