...
instance Eq StreamGraph where
    a == b = fromStreamGraph a == fromStreamGraph b

------------------------------------------------------------------------------
-- StreamProg type
-- an alternative StreamGraph representation to address an issue with Eq

data StreamProg = StreamProg Int StreamOperator [Exp] String String Double [StreamProg]
    deriving (Show)
...
prop_tofromStreamVertex_idempotent sv = sv == toStreamVertex (fromStreamVertex sv [])
...
prop_tofromStreamGraph_idempotent sg = 
