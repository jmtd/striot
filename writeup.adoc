...
scanExpandPost2 = streamExpand . streamScan (\b a -> scanl (\c _ -> c+1) (1 + last b) a) [0::Int]
-- very close, but it outputs too much (furthr array contents after end of data)

prop_scanExpand :: Stream [Char] -> Bool
prop_scanExpand s = scanExpandPre s == scanExpandPost2 s
------------------------------------------------------------------------------
\end{code}
